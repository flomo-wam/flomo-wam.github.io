...
| Path | What |
|---|---|
| `deck/index.html` | Slide deck (14 slides, self-contained). Open it in Chrome. Keys: arrow keys = step / slide, `N` = speaker notes, `O` = overview, `#n` in the URL = go to slide n. |
| `videos/FloMo_v3_master.mp4` | Full video, 130 s, 1080p |
| `videos/FloMo_v3_x.mp4` | X/Twitter cut, 113 s (drops RoboTwin + "motion as an action intermediate") |
| `videos/FloMo_v3_icra.mp4` | ICRA accompanying video, 130 s, under 20 MB, anonymous end card |
...
| `motion_hc.py` | Renders `media/motion/` from SpatialTrackerV2 tracks (training targets, not predictions). Slide 3's scene-flow raster = the training target (cumulative displacement on the 64x64 query grid) |
